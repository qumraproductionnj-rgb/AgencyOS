...
import { useQuotations, useUpdateQuotationStatus, useDeleteQuotation } from '@/hooks/use-quotations'
...
export function QuotationList() {
  ...
  return (
    <div className="space-y-4">
      ...
      {!quotations?.length ? (
        <p className="text-muted-foreground p-8 text-center">{t('noQuotations')}</p>
      ) : (
            ...
            <tbody className="divide-y">
              {quotations.map((q) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">{formatCurrency(Number(q.total), q.currency)}</td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[q.status]}`}
                    >
                      {t(STATUS_LABELS[q.status])}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    {q.status === 'DRAFT' && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          if (window.confirm(t('deleteConfirm'))) deleteQuotation.mutate(q.id)
                        }}
      ...
      {detailId && <QuotationDetail quotationId={detailId} onClose={() => setDetailId(null)} />}
    </div>
  )
}
