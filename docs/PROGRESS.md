...
**Current Task:** 2.14 — Notifications System
...
- [x] Prisma schema: `Project` model with stage enum, JSON arrays (revisions, reviews, deliverables), belongs to `Client` (required), optional `Campaign`
- [x] Migration `20260501000000_projects` applied
- [x] Backend DTOs: `CreateProjectDto`, `UpdateProjectStageDto`, `AddRevisionDto`, `ProjectQueryDto` with `campaignId` filter
- [x] `project.service.ts` — full CRUD, forward-only stage validation, `sendBackToInProgress` rollback (max 3 revisions), revision increment
- [x] `project.controller.ts` — 7 endpoints: POST, GET, GET:id, PATCH:id (update), PATCH:id/stage, PATCH:id/revision, DELETE:id
- [x] `project.module.ts` — registers service + controller
- [x] `project.service.spec.ts` — passes, covers CRUD, stage transitions, revision limits
- [x] Frontend `use-projects.ts` — 7 React Query hooks (list, get, create, update, updateStage, addRevision, delete)
- [x] `project-list.tsx` — filterable table with search, campaign filter, create button
- [x] `project-kanban.tsx` — stage columns with HTML5 drag-and-drop
- [x] `project-modal.tsx` — create/edit form with client/campaign pickers, stage select, date pickers, deliverables array
- [x] `project-detail.tsx` — full detail with stage progression buttons, revision request form (max 3), timeline, deliverable list, delete
- [x] Route page `app/[locale]/projects/page.tsx` — list/kanban toggle, header
- [x] Translations: AR + EN `projects` block (40+ keys: stage labels, actions, form fields, revision tracking)
- [x] `pnpm lint` ✓ | `pnpm typecheck` ✓ | `pnpm test` ✓ (183 tests, 21 suites)
- [x] Committed: `feat(projects): CRUD, forward-only stage transitions, revision tracking (max 3), AR/EN translations`

**Key decisions:**

- Project belongs to `Client` (required), not `Campaign` (optional): reflects real workflow where clients have projects outside campaigns
- Revisions stored as JSON array in `Project` row (not separate table): simplifies migration, max-3 enforcement easy
- Stage enum includes `CANCELLED` and `DELIVERED`; forward-only except `sendBackToInProgress` from REVIEW
- `exactOptionalPropertyTypes`-safe pattern: conditional spread instead of `|| undefined`
...
- [x] `expense.dto.ts` — 3 Zod schemas for Create, Update, Approve (status + rejectionReason)
- [x] `expense.service.ts` — CRUD, soft delete, auto-approval (IQD ≤150k / USD ≤100), approve/reject via `PATCH /expenses/:id/status`
- [x] `expense.controller.ts` — 6 endpoints (list, get, create, update, approve-status, delete); `@RequireRole('owner', 'admin', 'account_manager', 'project_manager')`
- [x] `expense.module.ts` — registered in `app.module.ts`
- [x] `expense.service.spec.ts` — 12 unit tests (findAll, findOne, create PENDING/auto-APPROVED, update, approve, reject with reason, non-pending reject, soft delete)
- [x] Frontend `use-expenses.ts` — 6 React Query hooks (list, get, create, update, approve, delete) + `useEmployeesList`
- [x] `expense-list.tsx` — table with search text, status dropdown, category dropdown filters
- [x] `expense-modal.tsx` — create/edit slideover with employee dropdown, category select, amount/currency, date picker
- [x] `expense-detail.tsx` — side panel with all fields, approve/reject buttons, inline rejection reason textarea
- [x] `app/[locale]/expenses/page.tsx` — route page shell
- [x] Translations: AR + EN `expenses` nav link + 25 keys (status labels, category labels, approval actions)
- [x] `pnpm lint` ✓ | `pnpm typecheck` ✓ | `pnpm test` ✓ (165 tests, 19 suites)

**Key decisions:**

- Auto-approval logic server-side: `shouldAutoApprove()` checks amount + currency thresholds
- Expense `employee` relation uses Employee's `fullNameAr`/`fullNameEn` (not User model — User has no `name` field)
- `approver` relation on Expense points to `User` (not Employee) — display `email` since no name on User
- Draft-only editing: `update()` throws `BadRequestException` if status is not PENDING
- `exactOptionalPropertyTypes` handled with conditional spread `...(condition ? { key: val } : {})` throughout
...
- Task 2.13 — Files Module (TUS chunked upload) ✅
...
- 2.14 — Notifications System (in-app + email, WebSocket, preferences)
