...
**Current Task:** 2.1 — Database Schema for Phase 2 (in progress)
...
### Sub-task: 2.2a — Leave API Controller & Module (2026-05-10)

- [x] `leave.controller.ts` — 7 endpoints: `POST /leaves` (create), `GET /leaves` (list with filters), `GET /leaves/balance`, `GET /leaves/:id`, `PATCH approve`, `PATCH reject`, `PATCH cancel`
- [x] `leave.module.ts` — imports `PermissionModule` for role fetching, exports `LeaveService`
- [x] `app.module.ts` — `LeavesModule` registered
- [x] Fixed pre-existing TS errors in `leave.service.ts` — `leaveType` parameters typed as `LeaveType` enum instead of `string`
- [x] `pnpm lint` ✓ | `pnpm typecheck` ✓ | `pnpm test` ✓ (62 tests pass)

**Remaining for 2.2:**

- Unit tests for `leave.service.ts`
- Frontend: employee leave request form, HR approval queue, calendar view
- Notification triggers (submission/approval/rejection)
...
- Task 2.2 — HR: Leaves Management (backend: controller + module complete, frontend pending)
...
- 2.2 — HR: Leaves Management (API: leave request CRUD, approve/reject, balance calculation; Frontend: request form, approval queue, calendar)
